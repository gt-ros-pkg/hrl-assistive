...
def main():
    ...
    model.load_weights('/home/mpark/bagfiles/data_experiment_test/models/audio-31-2.21.hdf5')
...
#    for start in tqdm(range(0, 256, 5)):
        seq = y[start: maxlen]  
        seq_matrix = np.zeros((maxlen, nb_output), dtype=bool) 
        for i,s in enumerate(seq):
            sample_ = int(s * (nb_output - 1)) # 0-255
    ...
    librosa.output.write_wav('data_seq.wav', seqA2, sr)
