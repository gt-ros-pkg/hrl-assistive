...
# TODO: Add steady state detection for stationary gestures: feeding, scooping

# User interface to send commmands to feeding system using gestures.
# Feeding: Look straight at camera and keep mouth open for approx 3 seconds.
# Scooping: Look slightly to the right (where the spoon is) and open mouth for
#           until command is sent (approx 3 sec).
# Clean spoon: Look straight at camera, open mouth, and slowly rotate head to
#              right (i.e. a "dragging" motion, where opening mouth is a
#              "clicking" movement).
# Wipe mouth: Look straight at camera, open mouth, and slowly rotate head
#                   to left (same as "clean spoon" command, but left rotation).
#
...
class DlibFaceLandmarkDetector:
    ...
    def __init__(self, img_topic='/SR300/rgb/image_raw'):
        ...
        # Wiping mouth
        self.wipe_mouth_start = True
        self.wipe_mouth_outlier = 0
    ...
    def callback(self, data):
        ...
        if len(self.dets)>=1:
            ...
            # arrays in order of [ratio, velocity]?
            ratio_observed = ratio # ratio observed from image info
            velocity_observed = ratio - self.prev_state[0] # observed curr ratio - filtered prev ratio
            ...
            ratio_kalman = next_state[0] # filtered ratio
            ...
            if (steady2):
                if ratio_kalman < 0.4 and mouth_open:
                    ...
                    if (self.gui_status == 'select task') or (self.gui_status == 'stopped'):
                        ...
                        self.userInputPub.publish('Start')
                elif mouth_open and not head_rotated:
                    print 'feeding conditions met, gui status: {}'.format(self.gui_status)
                    cv2.putText(img, 'Feeding!', (2, 230), cv2.FONT_HERSHEY_PLAIN, 2, cv2_green, 2)
                    if (self.gui_status == 'select task') or (self.gui_status == 'stopped'):
                    ...
                    cv2.putText(img, 'Stop!', (2, 230), cv2.FONT_HERSHEY_PLAIN, 2, cv2_orange, 2)
            ...
            if not self.wiping_start:
                ...
                if (not mouth_open) or (next_state[1] > 0):
                    # next_state[1] > 0 is pos velocity, so means moved in other direction
                    if self.wiping_outlier < 3: self.wiping_outlier += 1
                    else: self.wiping_start = False
                elif ratio_kalman < 0.4:
                    #self.publish_wipe = True
                    print 'clean spoon condition met, gui status: {}'.format(self.gui_status)
                    cv2.putText(img, 'Clean spoon!', (2, 230), cv2.FONT_HERSHEY_PLAIN, 2, cv2_yellow, 2)
                    if (self.gui_status == 'select task') or (self.gui_status == 'stopped'):
            ...
            # Wipe mouth condition check
            # open, look left?
            if not self.wipe_mouth_start:
                if mouth_open and not head_rotated:
                    self.wipe_mouth_start = True
                    self.wipe_mouth_outlier = 0
            else:
                if (not mouth_open) or (next_state[1] < 0):
                    # next_state[1] < 0 is neg velocity, so means moved in other direction
                    if self.wipe_mouth_outlier < 3: self.wipe_mouth_outlier += 1
                    else: self.wipe_mouth_start = False
                elif ratio_kalman > 2.5:
                    #self.publish_wipe = True
            ...
            color = None
            # Conditions for feeding gesture met
            if mouth_open and not head_rotated:
                self.conditions_met = True
                self.outliers = 0
                color = cv2_green
            # Conditions for feeding gestures not met but were met before,
            # and this is the 5th frame or less of not meeting conditions
