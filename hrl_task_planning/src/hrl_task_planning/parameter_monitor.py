        ...
        for arg in self.args:
            ...
            if rospy.has_param(param):
                if pred not in self.state:
                    rospy.loginfo("[%s] Param %s now set -- Adding predicate %s", rospy.get_name(), param, pred)
                    self.state.append(pred)
                    pub = True
            else:
                try:
                    self.state.remove(pred)
                    rospy.loginfo("[%s] Param %s gone -- Removing predicate %s", rospy.get_name(), param, pred)
                    pred.negate()
                    self.state.append(pred)
                    pub = True
                except ValueError:
