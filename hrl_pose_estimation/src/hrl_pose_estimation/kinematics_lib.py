    ...
    def forward_kinematics_pytorch(self, images_v, torso_lengths_angles_v, targets_v, loss_vector_type, kincons_v = None, prior_cascade = None, forward_only = False, body_side = None):
        ...
        if loss_vector_type == 'upper_angles':
            ...
            if loop == True:
                ...
                torso_lengths_angles_v[:, 18] = torch.clamp(torch.add(torso_lengths_angles_v[:, 18], 0.3), -0.05, 0.6) #torso angle for upper
                torso_lengths_angles_v[:, 19] = torch.clamp(torch.add(torso_lengths_angles_v[:, 19], 0.1), -0.05, 0.6) #torso angle for lower
