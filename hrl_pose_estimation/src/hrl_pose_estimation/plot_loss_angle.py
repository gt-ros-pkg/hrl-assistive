...
class DataVisualizer():
    ...
    def validate_model(self):
        ...
        test_xa = self.pad_stack(test_xa)
    ...
    def pad_stack(self,NxCxHxWimages):
        padded = np.zeros((NxCxHxWimages.shape[0],NxCxHxWimages.shape[1],NxCxHxWimages.shape[2]+20,NxCxHxWimages.shape[3]+20))
        padded[:,:,10:74,10:37] = NxCxHxWimages
        NxCxHxWimages = padded
        return NxCxHxWimages
    ...
    def preprocessing_create_pressure_angle_stack(self, x_data, a_data):
        ...
        print len(x_data[0]), 'x', 1, 'size of an incoming pressure map'
        print len(p_map_dataset[0][0]), 'x', len(p_map_dataset[0][0][0]), 'size of a resized pressure map'
        print len(p_map_dataset[0][1]), 'x', len(p_map_dataset[0][1][0]), 'size of the stacked angle mat'
