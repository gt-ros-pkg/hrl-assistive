...
class CNN(nn.Module):
    def __init__(self, mat_size, out_size, hidden_dim, kernel_size):
        ...
        self.CNN_pack1 = nn.Sequential(
            torch.nn.Conv2d(2, hidden_dim1, kernel_size = 12, stride = 2, padding = 2),
            torch.nn.ReLU(inplace = True),
        )

        self.CNN_pack2 = nn.Sequential(
            torch.nn.Conv2d(hidden_dim1, hidden_dim2, kernel_size=6, stride=2, padding=2),
            torch.nn.ReLU(inplace=True),
        )

        self.CNN_pack3 = nn.Sequential(
            torch.nn.Conv2d(hidden_dim2, hidden_dim3, kernel_size=4, stride=1, padding=1),
            torch.nn.ReLU(inplace=True),
            #torch.nn.MaxPool2d(2,2), #this cuts the height and width down by 2
        )

        self.CNN_pack4 = nn.Sequential(
            torch.nn.Conv2d(hidden_dim3, hidden_dim4, kernel_size=4, stride=1, padding=1),
            torch.nn.ReLU(inplace=True),
        )


        self.CNN_pack5 = nn.Sequential(
            torch.nn.Conv2d(hidden_dim4, hidden_dim5, kernel_size=4, stride=1, padding=1),
            torch.nn.ReLU(inplace=True),
        )
