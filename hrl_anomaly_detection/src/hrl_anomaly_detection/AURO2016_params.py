...
def getFeeding(task, data_renew, HMM_renew, CF_renew, rf_center='kinEEPos',local_range=10.0, \
    ...
    if dim == 5:
        ...
        ROC_param_dict = {'nPoints': nPoints,\
                          'hmmgp_param_range':np.logspace(0.3, 2.2, nPoints)*-1.0+0.5, \
                          'progress_param_range': -np.logspace(-0.0, 1.5, nPoints)+0.1,\
                          'svm_param_range': np.logspace(-2.5, 0.7, nPoints),\
                          'hmmosvm_param_range': np.logspace(-3.0, 0., nPoints),\
                          'fixed_param_range': np.linspace(-1., 0.0, nPoints)-1.0,\
                          'change_param_range': np.logspace(0.5, 1.7, nPoints)*-1.0,\
