...
def getParams(task, bDataRenew, bHMMRenew, bCFRenew, dim, rf_center='kinEEPos',\
    ...
    if dim == 5:
        ...
        param_dict['ROC']['update_list'] = [ 'osvm', 'hmmosvm', 'hmmgp']
...
def getFeeding(task, data_renew, HMM_renew, CF_renew, rf_center='kinEEPos',local_range=10.0, \
    ...
    if dim == 5:
        ...
        ROC_param_dict = {'nPoints': nPoints,\
                          'hmmgp_param_range':np.logspace(-0.7, 2.2, nPoints)*-1.0+0.5, \
        ...
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 0.5, 'scale': 18.88,\
                          'add_logp_d': False}
        ...
        ROC_param_dict = {'nPoints': nPoints,\
                          'hmmgp_param_range':np.logspace(0.2, 2.4, nPoints)*-1.0+0.5, \
