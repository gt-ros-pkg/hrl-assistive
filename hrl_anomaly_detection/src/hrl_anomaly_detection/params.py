...
def getFeeding(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, ae_swtch=False, dim=4):
    ...
    if dim == 4:
        ...
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 2.555, 'scale': 6.0}
        SVM_param_dict = {'renew': False, 'w_negative': 0.825, 'gamma': 5.0, 'cost': 2.25,\
                          'hmmosvm_nu': 0.00316,\
        ...
        ROC_param_dict = {'methods': ['progress_time_cluster', 'svm','fixed', 'hmmosvm','hmmsvm_diag'],\
                          ...
                          'svm_param_range': np.logspace(-1.8, 1.5, nPoints),\
                          'hmmsvm_diag_param_range': np.logspace(-4, 1.2, nPoints),\
                          'hmmosvm_param_range': np.logspace(-4.0, 1.0, nPoints),\
        ...
        SVM_param_dict = {'renew': False, 'w_negative': 0.1, 'gamma': 0.1, 'cost': 1.0,\
                          'hmmosvm_nu': 0.00316}
...
def getPushingMicroBlack(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False,\
    ...
    if dim == 5:
        ...
        nPoints        = 20  # 'progress_time_cluster',,'fixed' , 'svm' , 
        ROC_param_dict = {'methods': [ 'change', 'fixed', 'progress_time_cluster', 'svm', 'hmmsvm_diag', 'hmmosvm' ],\
                          'update_list': [ 'change' ],\
                          'nPoints': nPoints,\
