...
def getScooping(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False,\
    ...
    if dim == 4:
        ...
        ROC_param_dict = {'methods': [ 'change', 'fixed', 'progress_time_cluster', 'svm', 'hmmosvm', 'hmmsvm_diag' ],\
...
def getFeeding(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, ae_swtch=False, dim=4):
    ...
    if dim == 4:
        ...
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 4.111, 'scale': 6.0}
        SVM_param_dict = {'renew': False, 'w_negative': 0.825, 'gamma': 3.911, 'cost': 4.75,\
                          'hmmosvm_nu': 0.000316,\
                          'hmmsvm_diag_w_negative': 0.525, 'hmmsvm_diag_cost': 10.0, \
                          'hmmsvm_diag_gamma': 1.5}

        nPoints        = 20
        ROC_param_dict = {'methods': ['progress_time_cluster', 'svm','fixed', 'hmmosvm','hmmsvm_diag'],\
                          'update_list': ['hmmosvm'],\
                          'nPoints': nPoints,\
                          'progress_param_range': -np.logspace(0., 1.5, nPoints),\
                          'svm_param_range': np.logspace(-3.5, 0.5, nPoints),\
                          'hmmsvm_diag_param_range': np.logspace(-4, 1.2, nPoints),\
                          'hmmosvm_param_range': np.logspace(-5.0, 0.5, nPoints),\
        ...
        handFeatures = ['unimodal_ftForce', \
                        'crossmodal_artagEEDist', 'crossmodal_artagEEAng']
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 1.0, 'scale': 4.11}
        ...
        SVM_param_dict = {'renew': False, 'w_negative': 0.825, 'gamma': 5.0, 'cost': 6.0,\
                          'hmmosvm_nu': 0.001}
...
def getPushingMicroWhite(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False, \
    ...
    if dim == 5:
        ...
        if ae_swtch:
            ...
            HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 5.0, 'scale': 7.33}
        ...
        SVM_param_dict = {'renew': False, 'w_negative': 1.175, 'gamma': 1.0, 'cost':5.0,\
                          'hmmosvm_nu': 0.01}
        ...
        nPoints        = 20  #
        ROC_param_dict = {'methods': ['fixed', 'progress_time_cluster', 'svm', 'hmmosvm', 'hmmsvm_diag'],\
                          'update_list': [ 'svm' ],\
                          'nPoints': nPoints,\
                          'progress_param_range':np.linspace(0.0, -8.0, nPoints), \
                          'svm_param_range': np.logspace(-2.5, 0, nPoints),\
...
def getPushingMicroBlack(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False,\
    ...
    if dim == 5:
        handFeatures = ['unimodal_ftForce',\
                        ...
                        'crossmodal_subArtagEEDist',\
                        'unimodal_audioWristRMS'] #'unimodal_audioPower', ,
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 6.83, 'scale': 9.0 }
        ...
        if ae_swtch:
            ...
            SVM_param_dict = {'renew': False, 'w_negative': 0.177, 'gamma': 0.9777, 'cost': 3.25,\
                              'hmmosvm_nu': 4.641e-05,
                              'hmmsvm_diag_w_negative': 0.2, 'hmmsvm_diag_cost': 12.5, \
                              'hmmsvm_diag_gamma': 0.507,\
                              'osvm_window_size': 10}

        nPoints        = 20  # 'progress_time_cluster',,'fixed' , 'svm' , 
        ROC_param_dict = {'methods': [ 'change', 'fixed', 'progress_time_cluster', 'svm', 'hmmsvm_diag', 'hmmosvm', 'osvm' ],\
                          'update_list': [ 'osvm' ],\
                          'nPoints': nPoints,\
