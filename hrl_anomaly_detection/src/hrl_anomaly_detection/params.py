...
def getParams(task, bDataRenew, bAERenew, bHMMRenew, dim, rf_center='kinEEPos',\
    ...
    if dim == 4:
        ...
        param_dict['ROC']['update_list'] = [ 'progress', 'rnd' ]
...
def getPushingMicroWhite(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False, \
    ...
    if dim == 5:
        ...
        ROC_param_dict = {'methods': [ 'change','fixed','progress', 'progress_state', \
                          ...
                          'update_list': [ 'hmmgp' ],\
                          'nPoints': nPoints,\
                          'progress_param_range':np.logspace(-0.7, 1.1, nPoints)*-1.0+0.5, \
                          ...
                          'rnd_param_range': 1.0-np.logspace(-1, 0.0, nPoints)+0.1,\
                          'cssvm_param_range': np.logspace(-4.0, 2.0, nPoints),
                          'sgd_param_range': np.logspace(-4, 1.2, nPoints) }        
        ...
        ##                   'add_logp_d': False}
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 0.52, 'scale': 7.,\
                          'add_logp_d': False}
