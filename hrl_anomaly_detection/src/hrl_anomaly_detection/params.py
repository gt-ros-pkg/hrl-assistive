...
def getParams(task, bDataRenew, bAERenew, bHMMRenew, dim, rf_center='kinEEPos',\
    ...
    if dim == 4:
        param_dict['ROC']['methods'] = [ 'fixed', 'change', 'progress', 'progress_state', 'progress_diag', \
                                         'osvm', 'hmmosvm', 'kmean', 'progress_redu']
...
def getFeeding(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, ae_swtch=False, dim=4,\
    ...
    if dim == 4:
        ...
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 6.0, 'scale': 9.22, \
                          'add_logp_d': False}
        ...
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 8.0, 'scale': 3.0, \
                          'add_logp_d': False}
...
def getPushingMicroWhite(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False, \
    ...
    if dim == 5:
        ...
        ROC_param_dict = {'methods': [ 'change','fixed','progress', 'progress_state', \
                          ...
                          'progress_redu_param_range': np.logspace(-4.0, 1.5, nPoints),\
