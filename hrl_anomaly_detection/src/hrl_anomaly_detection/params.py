...
def getScooping(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False,\
    ...
    if dim == 4:
        ...
        SVM_param_dict = {'renew': False, 'w_negative': 0.1, 'gamma': 1.291, 'cost': 3.5,\
                          'hmmosvm_nu': 0.00316,\
...
def getFeeding(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, ae_swtch=False, dim=4):
    ...
    if dim == 4:
        ...
        nPoints        = 20 #, 'hmmosvm','hmmsvm_diag'
        ROC_param_dict = {'methods': ['progress_time_cluster', 'svm','fixed', 'change'],\
                          'update_list': ['svm'],\
                          'nPoints': nPoints,\
