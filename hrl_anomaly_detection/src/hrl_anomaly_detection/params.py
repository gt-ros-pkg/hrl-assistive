...
def getPushingMicroWhite(task, data_renew, AE_renew, HMM_renew, rf_center,local_range, pre_train=False, dim=3):
    ...
    if dim == 5:
        ...
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 2.75, 'scale': 1.625}
    elif dim == 4:
        handFeatures = ['unimodal_ftForce',\
                        'crossmodal_artagEEDist',\
                        'crossmodal_subArtagEEDist',\
        ...
        handFeatures = ['unimodal_ftForce',\
                        ...
                        'unimodal_audioWristRMS'] #'unimodal_audioPower', ,
        SVM_param_dict = {'renew': False, 'w_negative': 6.0, 'gamma': 0.173, 'cost': 4.0}
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 3.8, 'scale': 5.0}
    elif dim == 2:
        handFeatures = ['unimodal_ftForce',\
                        'unimodal_audioWristRMS'] #'unimodal_audioPower', ,
        SVM_param_dict = {'renew': False, 'w_negative': 6.0, 'gamma': 0.173, 'cost': 4.0}
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 2.75, 'scale': 2.75}
    ...
    ROC_param_dict = {'methods': ['svm' ],\
                      ...
                      'svm_param_range': np.logspace(-1, 0.0, nPoints),\
                      'fixed_param_range': np.linspace(1.0, -3.0, nPoints),\
