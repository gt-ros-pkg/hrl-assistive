...
def tableOfConfusionOnline(hmm, normalTestData, abnormalTestData, c=-5, verbose=False):
    ...
    for i in xrange(len(normalTestData[0])):
        ...
        for j in range(2, len(normalTestData[0][i])):
            with suppress_output():
                anomaly, error = hmm.anomaly_check([x[i][:j] for x in normalTestData], c)
    ...
    for i in xrange(len(abnormalTestData[0])):
        ...
        for j in range(2, len(abnormalTestData[0][i])):
            with suppress_output():
                anomaly, error = hmm.anomaly_check([x[i][:j] for x in abnormalTestData], c)
...
def likelihoodOfSequences(hmm, trainData, thresTestData=None, normalTestData=None, abnormalTestData=None,
    ...
    for data, color in zip(dataSet, colors):
        ...
        for i in xrange(len(data[0])):
            ...
            for j in range(2, len(data[0][i])):
                ...
                ## exp_logp = hmm.expLikelihoods([x[i][:j] for x in data], minThresholds)
    ...
    if save_pdf:
        ...
        fig.savefig(os.path.join(os.path.dirname(__file__), 'test.pdf'))
...
def iteration(downSampleSize=200, scale=[10,10,10,10], nState=20, cov_mult=1.0, anomaly_offset=0.0, verbose=False,
    ...
    if savedDataFile is not None:
        ...
        with open(savedDataFile, 'rb') as f:
            ...
            hmm = learning_hmm_multi_n(nState=nState, nEmissionDim=4, anomaly_offset=anomaly_offset, verbose=verbose)
            hmm.fit(xData=trainData, ml_pkl=ml_pkl, use_pkl=use_pkl, cov_mult=[cov_mult]*16)
    ...
    # hmm = learning_hmm_multi_4d(nState=nState, nEmissionDim=4, anomaly_offset=anomaly_offset, verbose=verbose)
    hmm = learning_hmm_multi_n(nState=nState, nEmissionDim=4, anomaly_offset=anomaly_offset, verbose=verbose)
    ret = hmm.fit(xData=trainData, ml_pkl=ml_pkl, use_pkl=use_pkl, cov_mult=[cov_mult]*16)
    ...
    if not findThresholds:
        ...
        if not saveData:
            ...
            # fileName = '/home/zerickson/catkin_ws/src/hrl-assistive/hrl_anomaly_detection/src/hrl_anomaly_detection/hmm/dataFiles/%s_%d_%d_%d_%d.pkl'  % (taskName, downSampleSize, scale[0], nState, int(cov_mult))
