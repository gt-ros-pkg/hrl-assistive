...
def getFeeding(task, data_renew, HMM_renew, CF_renew, rf_center='kinEEPos',local_range=10.0, \
    ...
    if dim == 4:
        ...
        ROC_param_dict = {'nPoints': nPoints,\
                          'hmmgp_param_range': -np.logspace(-0.2, 2.0, nPoints), \
                          'progress_param_range': -np.logspace(-0.5, 1.4, nPoints)+1.0,\
                          'svm_param_range': np.logspace(-0.8, 2.5, nPoints),\
                          'bpsvm_param_range': np.logspace(-2, 0, nPoints),\
                          'hmmosvm_param_range': np.logspace(-5.0, 0.6, nPoints),\
        ...
        HMM_param_dict = {'renew': HMM_renew, 'nState': 25, 'cov': 0.1, 'scale': 12.33,\
                          'add_logp_d': False}
        ...
        ROC_param_dict = {'nPoints': nPoints,\
                          'hmmgp_param_range':np.logspace(0.0, 2.3, nPoints)*-1.0, \
                          'progress_param_range': -np.logspace(0.0, 1.4, nPoints),\
                          'svm_param_range': np.logspace(-2.5, 0.7, nPoints),\
                          'hmmosvm_param_range': np.logspace(-4.0, 0., nPoints),\
    ...
    ##                      'unimodal_ftForce_delta',\ #12
    ##                      'unimodal_ftForce_zero',\ #13
    ##                      'unimodal_ftForce',\ #14
    ##                      'unimodal_ftForceX', \ #15
    ##                      'unimodal_ftForceY', \ #16
    ##                      'unimodal_ftForceZ', \ #17
    ##                      'crossmodal_landmarkEEDist', \ #18
    ##                      'crossmodal_landmarkEEAng',\ #19
    ##                      'unimodal_fabricForce',\ #20
    ##                      'unimodal_landmarkDist'] #21
