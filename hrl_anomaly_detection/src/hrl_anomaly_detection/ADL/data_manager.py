...
def cutting_for_robot(d, dtw_flag=False):
    ...
    for j in xrange(len(ft_force_mag)-nZero):
        avg  = np.mean(ft_force_mag[j:j+nZero])
        avg2 = np.mean(ft_force_mag[j+1*nZero:j+2*nZero])

        if avg > ft_zero and idx_start is None and avg < avg2:
            idx_start = j #i+nZero
        if idx_start is not None:
            if avg < ft_zero and idx_end is None:
                idx_end = j+nZero
    ...
    for j, t in enumerate(audio_time):
        ...
        if t > end_time and a_idx_end is None:
            a_idx_end = j            
    a_idx_length = a_idx_end - a_idx_start
    ...
    for i, force in enumerate(ft_force_l):
        ...
        ft_time_cut      = np.array(ft_time[idx_start:idx_start+idx_length])
        ft_force_cut     = ft_force[:,idx_start:idx_start+idx_length]
        ft_torque_cut    = ft_torque[:,idx_start:idx_start+idx_length]
        ft_force_mag_cut = ft_force_mag[idx_start:idx_start+idx_length]

        start_time  = ft_time[idx_start]
        end_time    = ft_time[idx_start+idx_length]
        ...
        audio_time_cut = np.array(audio_time[a_idx_start:a_idx_start+a_idx_length])
        audio_data_cut = np.array(audio_data[a_idx_start:a_idx_start+a_idx_length])              
