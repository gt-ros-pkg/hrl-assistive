...
lrdecay_list=(1e-6 1e-7)
momentum_list=(1e-6 1e-7)
dampening_list=(1e-6 1e-7)
lambda_list=(1e-6 1e-7)
timewindow_list=(4)
batch_list=(1)
layersize_list=('[256,128,8]')
maxiter=20000
                    ...
                    do
                        ...
                        do
                            ...
                            do
                                ...
                                IN=$(THEANO_FLAGS=mode=FAST_RUN,device=gpu,floatX=float32 python test.py --train --ls ${n} --lr ${i} --lrd ${ii} --m ${j} --d ${jj} --lambda ${k} --tw ${l} --batch_size ${m} --mi $maxiter 2>&1 > $FILENAME )
