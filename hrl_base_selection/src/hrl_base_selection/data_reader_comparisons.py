...
from hrl_base_selection.score_generator_comparisons import ScoreGenerator
...
class DataReader(object):
    ...
    def __init__(self, input_data=None, subject='sub6_shaver', reference_options=['head'],
                 ...
                 pos_clust=5, ori_clust=1, tf_listener=None, seed=None):
        self.score_sheet = []
        ...
        # Set seed for sampling-based search methods.
        self.seed=seed
    ...
    def generate_score(self, method='toc', sampling='cma', viz_rviz=False, visualize=False, plot=False):
        cachedir = mkdtemp()
        ...
        filename = ''.join([pkg_path, '/data/', self.task, '_', self.model, '_',method,'_',sampling,
                            # '_real_expanded_',
                            '_score_data.pkl'])
        save_pickle(score_sheet, filename)
        save_file_name = 'scores_for_comparison.pkl'
        file_path = ''.join([pkg_path, '/data/'])
        file_list = os.listdir(file_path)
        if save_file_name in file_list:
            loaded_pkl = load_pickle(file_path+save_file_name)
            for key in score_sheet:
                loaded_pkl[key] = score_sheet[key]
            save_pickle(loaded_pkl, file_path+save_file_name)
        else:
            save_pickle(score_sheet, file_path + save_file_name)
