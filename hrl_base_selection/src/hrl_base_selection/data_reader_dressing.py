...
from score_generator_dressing import ScoreGeneratorDressing
from score_generator_dressing_full import ScoreGeneratorDressingFull
...
class DataReader(object):
    ...
    def __init__(self, input_data=None, reference_options=['head'],
                 model='green_kevin', task='hospital_gown'):
        self.score_sheet = []
    ...
    def receive_task(self, task, ref_options):
        self.task_dict = task
        self.reference_options = ref_options
    ...
    def generate_score(self, viz_rviz=False, visualize=False, plot=False):
        if self.task_dict is None:
            ...
            selector = ScoreGeneratorDressing(visualize=visualize, targets=mytargets, reference_names=myReferenceNames,
                                      goals=myGoals, model=self.model, task=self.task)#, tf_listener=self.tf_listener)
            # selector.set_human_arm('rightarm')
            # selector.set_pr2_arm('rightarm')
            if viz_rviz:
                selector.show_rviz()
            # selector.set_human_arm('rightarm')
            # selector.set_pr2_arm('rightarm')
            selector.receive_new_goals(myGoals, mytask, 'rightarm', 'rightarm', myReferenceNames)

            score_sheet = selector.handle_score_generation(plot=plot)
        else:
            selector = ScoreGeneratorDressingFull(visualize=visualize)
            score_sheet = selector.calculate_scores(self.task_dict, self.model, self.reference_options)
