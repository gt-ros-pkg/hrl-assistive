    ...
    def read_bag(self, subject, filename, method, visualize=False):
        ...
        for topic, msg, t in bag.read_messages():
            if topic == '/fsascan':
                ...
                latest_ground_truth_time = t
                latest_ground_truth_time = msg.header.stamp
                self.head_origin_callback(msg)
            if self.mat_sampled and self.ground_truth_sampled and np.abs(latest_scan_time.to_sec() - latest_ground_truth_time.to_sec())<0.1:
                if count == 0:
                    start_time_range = t
                    start_time_range = latest_ground_truth_time
                if count < 50:
                    count += 1
                ...
                if count == 50:
                    ...
                    time_range = latest_ground_truth_time - start_time_range
...
if __name__ == '__main__':
    ...
    # for subject in [4, 4,4,4,4,4,4,4]:
