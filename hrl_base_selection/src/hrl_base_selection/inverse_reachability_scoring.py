...
import rospy
import time
from hrl_base_selection.data_reader_cma import DataReader as DataReader_cma
from hrl_base_selection.data_reader import DataReader
from hrl_base_selection.data_reader_task import DataReader_Task
# from score_generator import ScoreGenerator
from hrl_base_selection.score_generator_cma import ScoreGenerator
...
class InverseReachabilityScoring(object):
    def __init__(self):
        rospack = rospkg.RosPack()
        self.pkg_path = rospack.get_path('hrl_base_selection')
        self.data_path = '/home/ari/svn/robot1_data/usr/ari/data/base_selection'

        irs = InverseReachabilitySetup(visualize=False, redo_ik=False,
    ...
    def find_best_base(self, goal_data, task, model, sampling_type):


        return None

    def run_comparisons(self, tasks, model, sampling_type):
        print 'I am running comparisons for using the ', model, ' model'
        for task in tasks:
            print 'I will use data from the ', task, ' task'
            for sampling_type in ['uniform', 'gaussian', 'cma']:
                print 'Using scores generated by sampling using', sampling_type, 'method'
                read_task_data = DataReader_Task(task, model, 'cma')
                raw_data, raw_num, raw_reference, raw_reference_options = read_task_data.reset_goals()
                read_data = DataReader_cma(reference_options=raw_reference_options, model=model, task=task)
                goal_data = read_data.generate_output_goals(test_goals=raw_data, test_number=raw_num,
                                                            test_reference=raw_reference)
                best_base = self.find_best_base(goal_data, raw_reference_options, task, model, sampling_type)
                ...
                save_name = ''.join([self.data_path, '/', task, '_', model, '_', sampling_type, '_mc_scores.log'])
...
if __name__ == "__main__":
    ...
    myTest = InverseReachabilityScoring()
    ...
    # base_config = np.hstack([a,b])
    # best_base = [base_config, [0.057329581427009745, 1.0, 0.36352068257210146]]
    myTest.run_comparisons(tasks, model, method_options)
