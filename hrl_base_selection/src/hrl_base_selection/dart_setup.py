...
class DartDressingWorld(pydart.World):
    def __init__(self, skel_file_name):
        ...
        if not 'nolegs' in skel_file_name and not 'participant0' in skel_file_name and not 'participant1' in skel_file_name:
        ...
        elif 'participant0' in skel_file_name or 'participant1' in skel_file_name:
            q['j_thigh_right_z'] = m.radians(90.)
            q['j_thigh_right_y'] = m.radians(0.)
            q['j_thigh_right_x'] = m.radians(0.)
            q['j_thigh_left_z'] = m.radians(90.)
            ...
            q['j_shin_right'] = m.radians(-80.)
            q['j_shin_left'] = m.radians(-80.)
        ...
        print 'skel file name', skel_file_name
        if 'fullbody_participant0_capsule.skel' in skel_file_name:
            print 'participant0 model being used'
            ...
            positions['j_pelvis1_x'] += (-0.01 - self.human_reference_center_floor_point[0])
            positions['j_pelvis1_y'] += (0. - self.human_reference_center_floor_point[1])
            positions['j_pelvis1_z'] += (1. - self.human_reference_center_floor_point[2])
        elif 'fullbody_henryclever_capsule.skel' in skel_file_name:
            print 'henryclever model being used'
            for joint in self.skel_joints:
                if joint['@name'] == 'j_bicep_left':
                    self.x_reference_joint_index = self.skel_joints.index(joint)
                    self.z_reference_joint_index = self.skel_joints.index(joint)
            ...
            self.estimate_center_floor_point()
            positions = self.human.positions()
            positions['j_pelvis1_x'] += (-0.08 - self.human_reference_center_floor_point[0])
            positions['j_pelvis1_y'] += (0. - self.human_reference_center_floor_point[1])
            positions['j_pelvis1_z'] += (1.01 - self.human_reference_center_floor_point[2])
        elif 'fullbody_participant1_capsule.skel' in skel_file_name:
            print 'participant1 model being used'
            for joint in self.skel_joints:
                if joint['@name'] == 'j_bicep_left':
                    self.x_reference_joint_index = self.skel_joints.index(joint)
                    self.z_reference_joint_index = self.skel_joints.index(joint)
        ...
        elif 'fullbody_participant3_capsule.skel' in skel_file_name:
            print 'participant3 model being used'
            for joint in self.skel_joints:
                if joint['@name'] == 'j_bicep_left':
                    self.x_reference_joint_index = self.skel_joints.index(joint)
                    self.z_reference_joint_index = self.skel_joints.index(joint)
        ...
        elif 'fullbody_participant6_capsule.skel' in skel_file_name:
            print 'participant6 model being used'
            for joint in self.skel_joints:
                if joint['@name'] == 'j_bicep_left':
                    self.x_reference_joint_index = self.skel_joints.index(joint)
                    self.z_reference_joint_index = self.skel_joints.index(joint)
            ...
            positions['j_pelvis1_x'] += (-0.08 - self.human_reference_center_floor_point[0])
            positions['j_pelvis1_y'] += (0. - self.human_reference_center_floor_point[1])
            positions['j_pelvis1_z'] += (0.96 - self.human_reference_center_floor_point[2])
            ...
            for bodypart in self.skel_bodies:
                if bodypart['@name'] == self.skel_joints[self.z_reference_joint_index]['child']:
    ...
    def estimate_center_floor_point(self):
        x_position = self.human.joint(self.skel_joints[self.x_reference_joint_index]['@name']).position_in_world_frame()[0]
        y_position = self.human.joint(self.skel_joints[self.y_reference_joint_index]['@name']).position_in_world_frame()[1]
        z_position = self.human.joint(self.skel_joints[self.z_reference_joint_index]['@name']).position_in_world_frame()[2] - self.joint_to_floor_z
