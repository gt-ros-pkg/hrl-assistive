...
class MoveArmIntermediary():
    ...
    def __init__(self):
        ...
        rospy.Subscriber('wt_right_arm_pose_commands', PoseStamped, self.choose_method)
        rospy.Subscriber('wt_right_arm_angle_commands', JointTrajectoryPoint, self.send_joint_angles)
        rospy.Subscriber('wt_move_right_arm_goals', PoseStamped, self.compose_goal)
    
        self.pose_out = rospy.Publisher('r_hand_pose', PoseStamped)
        self.joints_out = rospy.Publisher('r_arm_controller/joint_trajectory_action/goal', JointTrajectoryActionGoal )
            ...
            if (state == 3): #3 == SUCCEEDED
                ...
                if result.error_code.val == 1:
                    ...
                    if torso_succeeded:
                        ...
                        rospy.loginfo("Failure Result: %s" %result)
                        self.wt_log_out.publish(data="Move Right Arm with Motion Planning: Failed: %s :Including Torso Movement Check" %self.move_arm_error_dict[result.error_code.val])
                else:
                    rospy.loginfo("Move_right_arm action failed: %s" %state)
                    rospy.loginfo("Failure Result: %s" %result)
