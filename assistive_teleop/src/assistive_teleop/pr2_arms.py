...
class PR2Arm():
    ...
    def __init__(self, arm, tfListener=None):
        ...
        rospy.loginfo(
            "Waiting for " + self.arm.capitalize() + " FK/IK Solver services")
        try:
    ...
    def build_trajectory(self, finish, start=None, ik_space=0.005,
        ...
        # if given one pose, use current position as start, else, assume
        # (start, finish)
        if start is None:
            start = self.curr_pose()

        # TODO: USE TF TO BASE DISTANCE ON TOOL_FRAME MOVEMENT DISTANCE, NOT
        # WRIST_ROLL_LINK
        ...
        ik_fracs = np.linspace(0, 1, ik_steps)
        ...
        # For each step between start and finish, find a complete pose (linear
        # position changes, and quaternion slerp)
        ...
        # Linearly interpolate angles 10 times between ik-defined points
        # (non-linear in cartesian space, but this is reduced from dense ik
        # sampling along linear path.  Used to maintain large number of
        # trajectory points without running IK on every one.
        angle_points = np.zeros((7, tot_points))
        for i in xrange(7):
            angle_points[i, :] = np.interp(
                ang_fracs, ik_fracs, ik_points[:, i])
    ...
    def build_follow_trajectory(self, traj):
        # Build 'Follow Joint Trajectory' goal from trajectory (includes
        ...
        if not (finished_within_time):
            ...
            if (state == 3):  # 3 == SUCCEEDED
                ...
                if result.error_code.val == 1:
                    ...
                    rospy.loginfo(
                        "Move_" + self.arm + "_arm action failed: %s" % state)
                    rospy.loginfo("Failure Result: %s" % result)
                    self.log_out.publish(data="Move " + self.arm.capitalize() +
                                         " Arm with Motion Planning \
                                                Failed: %s"
