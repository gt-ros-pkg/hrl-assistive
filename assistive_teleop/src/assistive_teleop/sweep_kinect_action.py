...
from assistive_teleop.msg import HeadSweepAction, HeadSweepActionResult
...
class HeadSweepActionServer(object):
    def __init__(self):
        ...
        self.head_joint_action = actionlib.simpleActionClient("/head_traj_controller/joint_traectory_action", JointTrajectoryAction)
        while not self.head_joint_action.wait_for_server(rospy.Duration(5)):
            rospy.loginfo("[%s] Waiting for head joint trajectory action server", rospy.get_name())
        ...
        def execute_cb(self, goal_msg):
            ...
            try:
                ...
                self.set_environment.call(merged_point_cloud)
            except Exception as e:
                self.report_failure(e)

            # Return result
            result = HeadSweepActionResult()
