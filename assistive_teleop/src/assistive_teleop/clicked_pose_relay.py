...
class ClickedPoseRelay(object):
    def __init__(self, translation, quaternion):
        ...
        rospy.loginfo('[clicked_pose_relay] Pose relay node started with:\r\nTranslation: %s\r\nRotation: %s' %(translation, quaternion))

    def pose_in_cb(self, ps_in):
        """Apply transform to received pose and republish"""
        trans_pose = pu.pose_relative_trans(ps_in, self.offset_x, self.offset_y, self.offset_z)
...
if __name__=='__main__':
    parser = argparse.ArgumentParser(description="Apply a transform (in the 'frame' of the pose) to an incoming pose and republish")
    parser.add_argument('-t','--translation', nargs=3, default=(0.,0.,0.), type=float,
                        help='The translation to be applied to the pose (with respect to itself)')
    parser.add_argument('-q','--quaternion', nargs=4, default=(0.,0.,0.,1.), type=float,
                        help='The rotation quaterion to be applied to the pose')
    ...
    if args[0].rotation:
        ...
        if args[0].quaternion != (0.,0.,0.,1.):
            parser.exit(status=1,message="Please specify only one form of rotation (received both RPY and Quaternion)")
        else:
