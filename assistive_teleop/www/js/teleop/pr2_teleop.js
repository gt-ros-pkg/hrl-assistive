...
function initGripper(orientation) {
    ...
    var rGripperStateDisplay = function (msg) {
        if ($('#r_gripper_slider > .ui-slider-handle').hasClass('ui-state-active') !== true) {
            $('#r_gripper_slider').show().slider('option', 'value', window.gripper[1].state);
        }
    };
    window.gripper[1].stateCBList.push(rGripperStateDisplay);
    $('#r_gripper_slider').unbind("slidestop").bind("slidestop", function (event, ui) {
        window.gripper[1].setPosition($('#r_gripper_slider').slider("value"));
        if ($('#r_gripper_slider').slider("value") > window.gripper[1].state) {
    ...
    var lGripperStateDisplay = function (msg) {
        if ($('#l_gripper_slider > .ui-slider-handle').hasClass('ui-state-active') !== true) {
            $('#l_gripper_slider').show().slider('option', 'value', window.gripper[0].state);
        }
    };
...
function teleopHead() {
    ...
    $('#bpd_default #b3').show().removeClass('arrow_rot_x_pos').text("Track Right Hand").bind('click.rfh', function (e) {
        window.clearInterval(window.head.pubInterval);
        window.head.pubInterval = setInterval(function () {
            window.head.pointHead(0, 0, 0, 'r_gripper_tool_frame');
        }, 100);
    });
    ...
    $('#bpd_default #b1').show().removeClass('arrow_rot_x_neg').text("Track Left Hand").bind('click.rfh', function (e) {
        window.clearInterval(window.head.pubInterval);
        window.head.pubInterval = setInterval(function () {
            window.head.pointHead(0, 0, 0, 'l_gripper_tool_frame');
        }, 100);
    });
}
...
function teleopBase() {
    ...
    $('#scale_slider').unbind("slidestop").bind("slidestop", function (event, ui) {
        window.base.scaleSlider = $('#scale_slider').slider("value");
    });
    ...
    $('#bpd_default #b8').show().bind('mousedown.rfh', function (e) {
        window.base.drive("#bpd_default #" + e.target.id, 0.2 * window.base.scaleSlider, 0, 0);
    });
    $('#bpd_default #b6').show().bind('mousedown.rfh', function (e) {
        window.base.drive("#bpd_default #" + e.target.id, 0, -0.2 * window.base.scaleSlider, 0);
    });
    ...
    $('#bpd_default #b3').show().addClass('arrow_rot_x_pos').bind('mousedown.rfh', function (e) {
        window.base.drive("#bpd_default #" + e.target.id, 0, 0, -0.6 * window.base.scaleSlider);
    });
