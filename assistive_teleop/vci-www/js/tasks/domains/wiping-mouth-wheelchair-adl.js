...
RFH.Domains.WipingMouthWheelchairADL = function (options) {
    'use strict';
    var self = this;
    var ros = options.ros;
    self.name = options.name || 'wiping_mouth_adl';
    self.domain = 'wiping_mouth_adl'
    var $button = $('#wiping-mouth-wheelchair-button');
    ...
    self.sendTaskGoal = function (side, goal) {
        ...
        self.setDefaultGoal(['(TASK-COMPLETED WIPING_MOUTH WHEELCHAIR)']);
        self.updatePDDLState(['(NOT (CONFIGURED BED WIPING_MOUTH WHEELCHAIR))', 
                              '(NOT (BASE-SELECTED WIPING_MOUTH WHEELCHAIR))',
                              '(NOT (IS-TRACKING-TAG WHEELCHAIR))',
                              '(NOT (CONFIGURED SPINE WIPING_MOUTH WHEELCHAIR))',
                              '(NOT (HEAD-REGISTERED WHEELCHAIR))',
                              '(NOT (OCCUPIED WHEELCHAIR))',
                              '(NOT (FOUND-TAG WHEELCHAIR))',
                              '(NOT (BASE-REACHED WIPING_MOUTH WHEELCHAIR))',
                              '(NOT (ARM-REACHED WIPING_MOUTH WHEELCHAIR))',
                              '(NOT (ARM-HOME WIPING_MOUTH WHEELCHAIR))',
                              '(NOT (TOO-CLOSE WHEELCHAIR))',
                              '(NOT (TASK-COMPLETED WIPING_MOUTH WHEELCHAIR))']);
        msg.goal = []; 
