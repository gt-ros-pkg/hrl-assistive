...
class tool_audio(Thread):
    ...
    def stopWav(self, file_name):
        ...
        for wavs in wav_list:
            if wavs.find(self.file_name)>=0:
                num = int(wavs.split('_')[-1].split('.')[0])
                if max_num < num:
                    max_num = num
        max_num = int(max_num)+1
        self.wav_name = self.file_name+'_'+str(max_num)+'.wav'

        print "Wav file name: ", self.wav_name

        wf = wave.open(self.wav_name, 'wb')
...
class ADL_log():
	...
	self.folder_name = '/home/'+ current_user + '/git/hrl-assistive/hrl_multimodal_anomaly_detection/recordings/'
        ...
        if change_folder == 'y':
            self.folder_name = raw_input("Enter new save folder: [ex: ../recordings/]")
