...
from assistive_teleop import pose_utils as pu
from hrl_face_adls.srv import PointMirror, PointMirrorResponse
...
class MirrorPointer(object):
    ...
    def get_pointed_mirror_pose(self):
        ...
        target_pt = PointStamped(
            self.head_pose.header, self.head_pose.pose.position)
    ...
    def point_mirror_cb(self, req):
        ...
        if not self.head_pose_sensible(self.head_pose):
            rospy.logwarn(
                "Registered Head Position outside of expected region: %s" % self.head_pose)
            return PoseStamped()
