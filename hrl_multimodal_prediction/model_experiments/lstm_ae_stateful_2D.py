...
TEST_SHIFT = 0
LOAD_WEIGHT = True
WEIGHT_FILE = './models/lstm-ae-stateful-2D-denoise-lab.h5'
PLOT = True
...
def define_network(batch_size, time_in, time_out, input_dim, n_neurons, load_weight=False):
	# model = Sequential()  
	inputs = Input(batch_shape=(batch_size, time_in, input_dim))
	encoded = LSTM(input_dim, return_sequences=False, activation='tanh', stateful=True)(inputs)        
	# we initiate these layers to reuse later.
	decoded_H2 = RepeatVector(time_out, name='H_2')
	decoded_L2 = LSTM(input_dim, return_sequences=True, activation='tanh', stateful=True, name='L_2')
	...
	print "Inputs: {}".format(lstm_ae.input_shape)
	print "Outputs: {}".format(lstm_ae.output_shape)
	
	if load_weight:
		lstm_ae.load_weights(WEIGHT_FILE)
	...
	lstm_ae.compile(loss=loss, optimizer=optimizer)
	return lstm_ae
